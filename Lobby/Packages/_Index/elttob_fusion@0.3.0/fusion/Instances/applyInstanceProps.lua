...
local function setProperty(p1, p2, p3) -- Line: 49 -- upvalues: setProperty_unsafe (val), parseError (val), testPropertyAssignable (val), External (val)
    local v1, v2
    v1, v2 = xpcall(setProperty_unsafe, parseError, p1, p2, p3)
    if v1 then
        return
    end
    ...
    local v3 = typeof(p3)
    local v4 = typeof(p1[p2])
    if v3 == v4 then
        External.logErrorNonFatal("propertySetError", v2)
        return
    end
    External.logErrorNonFatal("invalidPropertyType", nil, p1.ClassName, p2, v4, v3)
end
local function bindProperty(p1, p2, p3, p4) -- Line: 74 -- upvalues: castToState (val), checkLifetime (val), Observer (val), setProperty (val), peek (val)
    ...
    local v1 = Observer(p1, p4)
    v1:onBind(function() -- Line: 88 -- upvalues: setProperty (upval), p2 (val), p3 (val), peek (upval), p4 (val)
