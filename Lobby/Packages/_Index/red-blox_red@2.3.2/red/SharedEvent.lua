...
local function SharedBaseEvent(p1, p2) -- Line: 164
    -- upvalues: Identifier (val), FireClient (val), FireAllClients (val), FireAllClientsExcept (val), FireClients (val)
    -- upvalues: FireFilteredClients (val), FireServer (val)
    return {
...
return {
    SharedCallEvent = function(p1, p2) -- Line: 181
        -- upvalues: Identifier (val), FireClient (val), FireAllClients (val), FireAllClientsExcept (val)
        -- upvalues: FireClients (val), FireFilteredClients (val), FireServer (val), SetServerListener (val)
        -- upvalues: SetClientListener (val), RunService (val), Net (val), Spawn (val)
        local v1, v2
        ...
        local Name = v1.Name
        local v3 = v1.Unreliable or false
        local u25 = {
        ...
        u25.CallMode = "Call"
        u25.SetServerListener = SetServerListener
        u25.SetClientListener = SetClientListener
        u25.Listener = nil
        if RunService:IsServer() then
            v2 = Net
            v2.Server.SetListener(u25.Id, function(p1, p2_2) -- Line: 202 -- upvalues: Spawn (upval), u25 (val), p2 (val)
                local v1 = Spawn
                v1(function(p1, ...) -- Line: 203 -- upvalues: u25 (upval), p2 (upval)
                    if u25.Listener and pcall(p2, ...) then
                        u25.Listener(p1, ...)
                    end
                end, p1, table.unpack(p2_2))
            end)
            return u25
        end
        v2 = Net
        v2.Client.SetListener(u25.Id, function(p1) -- Line: 210 -- upvalues: Spawn (upval), u25 (val), p2 (val)
            local v1 = Spawn
            v1(function(...) -- Line: 211 -- upvalues: u25 (upval), p2 (upval)
                if u25.Listener and pcall(p2, ...) then
                    u25.Listener(...)
                end
            end, table.unpack(p1))
    ...
    SharedSignalEvent = function(p1, p2) -- Line: 222
        -- upvalues: Identifier (val), FireClient (val), FireAllClients (val), FireAllClientsExcept (val)
        -- upvalues: FireClients (val), FireFilteredClients (val), FireServer (val), Signal (val), OnServer (val)
        -- upvalues: OnClient (val), RunService (val), Net (val), Spawn (val)
        local v1, v2
        if typeof(p1) == "string" then
            v1 = {Unreliable = false}
            v1.Name = p1
        ...
        u25.CallMode = "Signal"
        u25.Signal = Signal()
        u25.OnServer = OnServer
        u25.OnClient = OnClient
        if RunService:IsServer() then
            v2 = Net
            v2.Server.SetListener(u25.Id, function(p1, p2_2) -- Line: 243 -- upvalues: Spawn (upval), p2 (val), u25 (val)
                local v1 = Spawn
                v1(function(p1, ...) -- Line: 244 -- upvalues: p2 (upval), u25 (upval)
                    if pcall(p2, ...) then
                        u25.Signal:Fire(p1, ...)
                    end
                end, p1, table.unpack(p2_2))
        ...
        v2 = Net
        v2.Client.SetListener(u25.Id, function(p1) -- Line: 251 -- upvalues: Spawn (upval), p2 (val), u25 (val)
            local v1 = Spawn
            v1(function(...) -- Line: 252 -- upvalues: p2 (upval), u25 (upval)
                if pcall(p2, ...) then
                    u25.Signal:Fire(...)
                end
            end, table.unpack(p1))
