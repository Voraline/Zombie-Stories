...
local v_u_6 = require("@self/Enum").enums
local v_u_7 = require("@self/Janitor")
local v_u_8 = require("@self/Signal")
local v9 = require("@self/ZonePlusReference")
...
function v_u_17._update(p_u_82) -- name: _update
	...
	elseif v86 == "Instance" then
		if v83:IsA("BasePart") then
			...
			for _, v89 in v83:QueryDescendants("BasePart") do
				table.insert(v84, v89)
			end
			for _, v90 in v83:QueryDescendants(":not(BasePart)") do
	...
	local v91 = true
	for _, v_u_92 in pairs(v84) do
		local _, v93 = pcall(function()
			-- upvalues: (copy) v_u_92
			return v_u_92.Shape.Name
		end)
		if v93 ~= "Block" then
			v91 = false
		end
	end
	p_u_82.allZonePartsAreBlocks = v91
	local v94 = OverlapParams.new()
	v94.FilterType = Enum.RaycastFilterType.Whitelist
	v94.MaxParts = #v84
	v94.FilterDescendantsInstances = v84
	p_u_82.overlapParams.zonePartsWhitelist = v94
	local v95 = OverlapParams.new()
	v95.FilterType = Enum.RaycastFilterType.Blacklist
	v95.FilterDescendantsInstances = v84
	p_u_82.overlapParams.zonePartsIgnorelist = v95
	local function v98() -- name: update
		-- upvalues: (copy) p_u_82, (ref) v_u_85, (ref) v_u_2
		if p_u_82.autoUpdate then
			local v_u_96 = os.clock()
			if p_u_82.respectUpdateQueue then
				v_u_85 = v_u_85 + 1
				v_u_96 = v_u_96 + 0.1
			end
			local v_u_97 = nil
			v_u_97 = v_u_2.Heartbeat:Connect(function()
				-- upvalues: (ref) v_u_96, (ref) v_u_97, (ref) p_u_82, (ref) v_u_85
				if v_u_96 <= os.clock() then
					v_u_97:Disconnect()
					if p_u_82.respectUpdateQueue then
						v_u_85 = v_u_85 - 1
					end
	...
	local v99 = { "Size", "Position" }
	for _, v_u_100 in pairs(v84) do
		for _, v101 in pairs(v99) do
			p_u_82._updateConnections:add(v_u_100:GetPropertyChangedSignal(v101):Connect(v98), "Disconnect")
		end
		if v_u_100.CollisionGroupId ~= 0 and v_u_100.CollisionGroupId ~= -1 then
		...
		p_u_82._updateConnections:add(v_u_100:GetPropertyChangedSignal("CollisionGroupId"):Connect(function()
			-- upvalues: (copy) v_u_100
			local v102 = v_u_100
			if v102.CollisionGroupId ~= 0 and v102.CollisionGroupId ~= -1 then
				error("Zone parts must belong to the \'Default\' (0) CollisionGroup! Consider using zone:relocate() if you wish to move zones outside of workspace to prevent them interacting with other parts.")
			end
		end), "Disconnect")
	end
