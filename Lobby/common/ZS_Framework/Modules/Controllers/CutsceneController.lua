...
local CutsceneEvent = require(RedEvents.General.CutsceneEvent)
local u30 = "Cinematic"
local u31 = false
local u33 = nil
local u34 = {}

function u34.SetCutsceneMode(p1, p2) -- Line: 22 -- upvalues: u31 (ref), u30 (ref)
...
function u34.SetCutsceneEnabled(p1, p2) -- Line: 29
    -- upvalues: CameraController (val), WeaponController (val), HUDController (val), LocalPlayerController (val)
    -- upvalues: u30 (ref), RunService (val), u31 (ref)
...
function setupFirstPersonCutscene() -- Line: 45 -- upvalues: RunService (val), u33 (ref)
    ...
    v1:BindToRenderStep("FirstPersonCutscene", Value, function() -- Line: 46 -- upvalues: u33 (upval)
        if u33 then
...
CutsceneEvent:SetClientListener(function(p1) -- Line: 54 -- upvalues: u34 (val)
    if p1 then
        ...
        if p1.Type == "SetCutsceneEnabled" then
            ...
            v1:SetCutsceneEnabled(Enabled)
            return
        end
        if p1.Type == "SetCutsceneMode" then
            v1 = u34
            local Mode = p1.Mode
            v1:SetCutsceneMode(Mode)
        end
    end
end)
