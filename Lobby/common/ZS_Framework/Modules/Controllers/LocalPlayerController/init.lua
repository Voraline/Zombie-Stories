...
local v5 = (CFrame.new(0, -0.5, 1.25)) * CFrame.Angles(0, 3.141592653589793, 0)
...
local function GetSafeFirstPersonOffset(p1, p2) -- Line: 129
    -- upvalues: u221 (ref), u220 (ref), u114 (val), u217 (ref), u219 (ref)
    local Unit, v1, v2
    local v3 = os.clock()
    local v4 = 0.1 < v3 - u221
    u221 = v3
    if u220 and not v4 then
        return (CFrame.new(0, 0, 1.25)) * u114
    end
    local v5 = workspace.CurrentCamera.CFrame.LookVector * Vector3.new(1, 0, 1)
    if not (0.3 < v5.Magnitude) then
        v2 = p1.CFrame.LookVector * Vector3.new(1, 0, 1)
    ...
    v2 = -Unit
    local v6 = p1.Position + Unit * 0.5
    local v7 = u217.CustomRayDirection(v6, v2 * 3, true)
    local v8 = -p1.CFrame.LookVector
    local v9 = u217.CustomRayDirection(p1.Position, v8 * 3, true)
    local v10 = 3
    if v7.Instance then
        v1 = v7.Distance - 0.5
        v10 = math.min(v10, v1)
    ...
    if v9.Instance then
        local Distance_2 = v9.Distance
        v10 = math.min(v10, Distance_2)
    end
    local v11 = v10 - 1
    v1 = math.max(0, v11)
    local v12 = math.min(1.25, v1)
    if not v4 then
        u219 = u219 + (v12 - u219) * 0.03
    else
        u219 = v12
        u220 = false
    end
    ...
    return (CFrame.new(0, 0, u219)) * u114
end
...
u232.humanoid = require(LocalPlayerUtils:WaitForChild("HumanoidUtil"))
u232.PlayerMovementUtil = require(PlayerMovementUtil)
u232.States = PlayerHandler:WaitForPlayerState(LocalPlayer)
u232.CharacterChanged = BindableEvent.Event
u232.StaminaChanged = Signal.new()
u232.ThirdPersonChanged = Signal.new()
u232.MovementEnabled = true
u232.LastForceTeleportClock = nil
u232.FocusEnabled = false
u232.SprintPressed = false
u232.CrouchPressed = false
u232.PronePressed = false
u232.RequestSlideJump = false
u232.RequestVault = false
...
Settings.SettingsChanged:Connect(function(p1) -- Line: 235 -- upvalues: u232 (val), peek (val), Settings (val)
    ...
    if p1 and p1[1] == "Graphics" and p1[2] == "ProceduralAnimations" and u232.Animator then
        ...
        local v3 = peek
        local v4 = Settings
        v3 = v3(v4.Graphics.ProceduralAnimations)
        Animator:SetIKEnabled(v3)
    end
end)
...
function u232.InfiniteStamina(p1) -- Line: 267 -- upvalues: u145 (ref), u146 (ref), u147 (ref), u148 (ref)
    ...
    local Cola = workspace.Cola
    SoundService:PlayLocalSound(Cola)
end

function u232.Init(p1) -- Line: 282
    -- upvalues: u217 (ref), PlayerHandler (val), u232 (val), Fusion (val), SkillTreeData (val), AddStamina (ref)
    -- upvalues: u42 (ref), LocalPlayer (val), BindableEvent (val), PhysBallUtil (val), SharedSprings (val)
    -- upvalues: RunService (val), u231 (val), ViewmodelManager (val), u148 (ref), u31 (ref), u34 (ref), u150 (ref)
    -- upvalues: u41 (ref), Resources (val), u40 (ref), GetSafeFirstPersonOffset (val), u144 (val), u46 (ref), u44 (ref)
    -- upvalues: u56 (ref), u52 (val), u47 (ref), u53 (ref), u55 (ref), u54 (ref), u61 (ref), u64 (ref), u62 (ref)
    -- upvalues: u63 (ref)
    ...
    v1.HealthChanged:Connect(function(p1, p2, p3) -- Line: 287
        -- upvalues: u232 (upval), u3 (val), Fusion (upval), SkillTreeData (upval), AddStamina (upval), u6 (val)
        -- upvalues: u42 (upval)
        u232:HPUpdated(p1)
        if 0 < p2 then
            local v1 = u3
            ...
            CameraShaker:ShakeOnce(7, 7, 0, 0.5, v2, (Vector3.new(1, 1, 1)))
            v1 = Fusion.peek(SkillTreeData.AdrenalineStamina) or 0
            if 0 < v1 then
                AddStamina(v1)
            end
            local LiveDamagePos = u6:GetLiveDamagePos(p3)
            if u232.CurrentWeapon and LiveDamagePos then
                local v3 = -(LiveDamagePos - u232.hrp.Position).unit
                local v4 = u232
                local lookVector = v4.hrp.CFrame.lookVector
                local v5 = v3:Dot(lookVector)
                v2 = u232
                local rightVector = v2.hrp.CFrame.rightVector
                local v6 = v3:Dot(rightVector)
                v4 = u232
                local Viewmodel = v4.CurrentWeapon.Viewmodel
                local v7 = CFrame.Angles(0.13962634015954636 * v5, -0.13962634015954636 * v6, -0.05235987755982989 * v6)
                Viewmodel:ApplyImpulse(v7)
            end
        end
        if p1 <= 0 then
            u42 = true
    ...
    v1 = LocalPlayer
    v1.CharacterRemoving:Connect(function() -- Line: 332 -- upvalues: BindableEvent (upval)
    ...
    v1 = u232
    v1.humanoid.Landed:Connect(function() -- Line: 340 -- upvalues: SharedSprings (upval)
        SharedSprings.YawSpring.Position = SharedSprings.YawSpring.Position + 0.05
    end)
    v1 = u232
    v1.humanoid.Jumped:Connect(function() -- Line: 343 -- upvalues: SharedSprings (upval)
        SharedSprings.YawSpring.Position = SharedSprings.YawSpring.Position - 0.05
    end)
    v1 = RunService
    v1.Stepped:Connect(function(p1, p2) -- Line: 348 -- upvalues: u232 (upval)
    ...
    v1 = RunService
    local v2 = u231
    v1:BindToRenderStep("LPC", v2, function(p1) -- Line: 354
        -- upvalues: u232 (upval), ViewmodelManager (upval), Fusion (upval), SkillTreeData (upval), u148 (upval)
        -- upvalues: LocalPlayer (upval), u31 (upval), u34 (upval), u150 (upval), u41 (upval), Resources (upval)
        -- upvalues: u40 (upval), GetSafeFirstPersonOffset (upval), u144 (upval), u46 (upval), u217 (upval), u44 (upval)
        -- upvalues: u56 (upval), u52 (upval), u47 (upval), u53 (upval), u55 (upval), u54 (upval), u61 (upval)
        -- upvalues: u64 (upval), u62 (upval), u63 (upval)
        GetNewPlayerCharacter()
        if u232.humanoid.Humanoid and u232.character then
            local v1
            StatesHandler()
            SlidingHandler(p1)
            VaultingHandler()
            local v2 = p1 * 10
            local v3 = -1 * v2
            local v4 = 1 - math.exp(v3)
            local v5 = math.clamp(v4, 0.01, 1)
            if u232.Animator then
                v4 = u232
                local Animator = v4.Animator
                v1 = u232
                local States = v1.States
                Animator:SetState(States)
                v4 = u232
                local Animator_2 = v4.Animator
                v3 = u232
                local MovementEnabled = v3.MovementEnabled
                Animator_2:SetLocomotionSuspended(not MovementEnabled)
                local Animator_3 = u232.Animator
                v1 = u232.CurrentWeapon ~= nil
                Animator_3:SetWeaponEquipped(v1)
                if u232.CurrentWeapon then
                    v4 = ViewmodelManager
                    v1 = u232
                    local CurrentWeapon = v1.CurrentWeapon
                    local Entry = v4:GetEntry(CurrentWeapon)
                    if Entry then
                        v2 = u232
                        local Animator_4 = v2.Animator
                        local Left = Entry.GrantedArms.Left
                        local Right = Entry.GrantedArms.Right
                        Animator_4:SetArmControl(Left, Right)
                    end
                end
            ...
            if not u232.States.Sprinting then
                v4 = LocalPlayer:GetAttribute("Skill_StaminaRegenMult") or 1
                v2 = 100 * (LocalPlayer:GetAttribute("Skill_StaminaMaxMult") or 1)
                if u31 < v2 then
                    v1 = os.clock()
                    if u34 < v1 then
                        v1 = os.clock()
                        if u150 < v1 then
                            setStamina(u31 + v5 * 2 * v4, v2 / 100)
                        end
                    end
                end
            elseif not u232.States.Jogging then
                v4 = Fusion.peek(SkillTreeData.DesperateSprintThreshold) or 0
                v2 = u232.States.HP or 100
                v1 = u232.States.MaxHP or 100
                if not (0 < v1) then
                    v3 = 1
                else
                    v3 = v2 / v1
            ...
            if not u232.RequestThirdPerson or u232.States.IsDead then
                updateThirdPerson(false)
                if u232.States.IsDead then
                    u232.RequestThirdPerson = false
                end
            elseif not u232.CurrentWeapon
                or not u232.CurrentWeapon.Config.AimFOVMultiplier
                or not (u232.CurrentWeapon.Config.AimFOVMultiplier <= 0.5)
                or not u232.States.Aiming then
                updateThirdPerson(true)
            else
                updateThirdPerson(false)
            end
            if u40 then
                local C0, C0_2, CustomRay, LookVector, Normal, Position, Unit, Unit_2, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15
                if not u232.ThirdPerson then
                    ...
                    if u232.States.IsDowned or u232.States.IsDead then
                        v4 = "Normal"
                    end
                ...
                v1 = not u232.ThirdPerson
                if v4 ~= "NormalFP" or not u232.hrp then
                    v2 = u144[v4]
                else
                    v2 = GetSafeFirstPersonOffset(u232.hrp, v1)
                end
                u46 = u46:Lerp(v2, v5)
                local hrp = u232.hrp
                if u232.States.Proning then
                    if not hrp or not hrp.Parent then
                        ...
                        v12 = u217
                        CustomRay = v12.CustomRay
                        Position = hrp.CFrame.Position
                        v14 = hrp.CFrame.Position + Vector3.new(0, -5, 0)
                        v12 = CustomRay(Position, v14, true)
                        if not v12.Instance then
                            ...
                            if u56 then
                                ...
                                C0_2 = v14.C0
                                v6 = u52
                                v13.C0 = C0_2:Lerp(v6, v5)
                            end
                        else
                            Normal = v12.Normal
                            LookVector = hrp.CFrame.LookVector
                            Unit = Normal:Cross(LookVector).Unit
                            Unit_2 = Unit:Cross(Normal).Unit
                            v6 = CFrame.new(
                                0,
                                0,
                                0,
                            ...
                            if u56 then
                                ...
                                C0 = v10.C0
                                v11 = u52 * v7
                                v9.C0 = C0:Lerp(v11, v5)
                            end
                        end
                    end
                elseif not u232.States.IsDowned or not hrp or not hrp.Parent then
                    v12 = u44
                    v14 = CFrame.new()
                    u44 = v12:lerp(v14, v5)
                else
                ...
                if u232.ThirdPerson then
                    ...
                    if not u232.States.Crouching then
                        if not u232.States.Sliding then
                            u40.C0 = v12 * CFrame.new(0, u47, 0)
                        else
                            u40.C0 = v12 * CFrame.new(0, 1, 0)
                        end
                    elseif not u232.States.Sliding then
                        v14 = (CFrame.new(0, u47, 0)) * CFrame.Angles(-1.5707963267948966, 0, 0)
                        v15 = CFrame.new(0, 0, 1)
                        v7 = (0.5 - u47) / 0.25
                        v6 = math.clamp(v7, 0, 1)
                        u40.C0 = v12 * v14:Lerp(v15, v6)
                    elseif not u232.States.Sliding then
                        u40.C0 = v12 * CFrame.new(0, u47, 0)
                    else
                        u40.C0 = v12 * CFrame.new(0, 1, 0)
                ...
                if hrp then
                    ...
                    local Proning = u232.States.Proning
                    if not Proning then
                        Proning = u232.States.Diving
                    end
                    if not Proning or not u53 then
                        if not Proning and u53 then
                            u61()
                        end
                    elseif u232.character then
                        for i, j in u232.character:GetChildren() do
                            if j:IsA("BasePart")
                                and j.Name ~= "ProneCollider"
                                and j.Name ~= "FPHeadCollider"
                                and j.Name ~= "Vehicle" then
                                j.CanCollide = false
                            end
                        end
                        if not Proning or not u55 or not u55.Parent then
                            u54 = Vector3.new(0, 0, 0)
                        else
                            local Position_2 = u55.Position
                            local Unit_3 = u232.hrp.CFrame.LookVector * Vector3.new(1, 0, 1)
                            if 0.01 < Unit_3.Magnitude then
                                Unit_3 = Unit_3.Unit
                            end
                            v7 = Vector3.new(0, 0, 0)
                            v8 = u217.CollisionRayDirection(Position_2, Unit_3 * 4)
                            if v8.Instance then
                                v9 = 4 - v8.Distance + 0.5
                                if 0 < v9 then
                                    v7 = v7 - Unit_3 * v9
                                end
                            end
                            v9 = u217.CollisionRayDirection(Position_2, -Unit_3 * 4)
                            if v9.Instance then
                                v10 = 4 - v9.Distance + 0.5
                                if 0 < v10 then
                                    v7 = v7 + Unit_3 * v10
                                end
                            end
                    ...
                    elseif not Proning and u53 then
                        u61()
                    end
                    v15 = not u232.ThirdPerson and not Proning
                    if not v15 then
                        if not v15 and u64 then
                            u63()
                        end
                    elseif not u64 then
                        u62()
                    elseif not v15 and u64 then
                        u63()
                    end
                end
            end
...
function u232.DrainStamina(p1, p2, p3) -- Line: 616 -- upvalues: LocalPlayer (val), u150 (ref)
    DrainStamina(p2)
    local Attribute = LocalPlayer:GetAttribute("Skill_StaminaCooldownMult")
    u150 = os.clock() + (p3 or 0) * (Attribute or 1)
end
...
;(LocalPlayer:GetAttributeChangedSignal("TurkeyHuntWalkSpeedBoostExpires")):Connect(function() -- Line: 643
    RefreshWalkspeedChanges()
end)
;(LocalPlayer:GetAttributeChangedSignal("Skill_StaminaMaxMult")):Connect(function() -- Line: 647 -- upvalues: LocalPlayer (val)
...
function setStamina(p1, p2) -- Line: 662 -- upvalues: LocalPlayer (val), u31 (ref), u232 (val)
    ...
    local v1 = 100 * Attribute
    local v2 = math.clamp(p1, 0, v1)
    if v2 ~= u31 then
        u232.StaminaChanged:Fire(v2)
    end
    u31 = v2
end
...
function DrainStamina(p1) -- Line: 671 -- upvalues: u232 (val), LocalPlayer (val), u31 (ref), u34 (ref)
    local v1
    local v2 = u232
    local IsFocused = v2.States.IsFocused
    local v3 = p1 * (LocalPlayer:GetAttribute("Skill_StaminaCostMult") or 1)
    if not IsFocused or not (0 < p1) then
        v1 = 1
    else
        v1 = 0.5
    end
...
function PlayerRespawned() -- Line: 690
    -- upvalues: u30 (ref), u232 (val), PhysBallUtil (val), u57 (ref), u56 (ref), u55 (ref), u53 (ref), u66 (ref)
    ...
    if u232.hrp and not u55 then
        ...
        local function addVehicleNoCollision(p1) -- Line: 753 -- upvalues: u55 (upval)
            if u55 and u55.Parent then
                if u55:FindFirstChild("VehicleNoCollision") then
                    return
                end
        ...
        local v1 = u232
        u57 = v1.character.ChildAdded:Connect(function(p1) -- Line: 772 -- upvalues: u55 (upval)
            if p1.Name == "Vehicle" and p1:IsA("BasePart") and u55 then
                if not u55.Parent or u55:FindFirstChild("VehicleNoCollision") then
                    return
                end
                local NoCollisionConstraint = Instance.new("NoCollisionConstraint")
                NoCollisionConstraint.Name = "VehicleNoCollision"
    ...
    if u232.hrp and not u64 then
        ...
        local function addFPHeadVehicleNoCollision(p1) -- Line: 844 -- upvalues: u64 (upval)
            if u64 and u64.Parent then
                if u64:FindFirstChild("FPHeadVehicleNoCollision") then
                    return
                end
                local NoCollisionConstraint = Instance.new("NoCollisionConstraint")
        ...
        local v1 = u232
        u66 = v1.character.ChildAdded:Connect(function(p1) -- Line: 863 -- upvalues: u64 (upval)
            if p1.Name == "Vehicle" and p1:IsA("BasePart") and u64 then
                if not u64.Parent or u64:FindFirstChild("FPHeadVehicleNoCollision") then
                    return
                end
                local NoCollisionConstraint = Instance.new("NoCollisionConstraint")
                NoCollisionConstraint.Name = "FPHeadVehicleNoCollision"
...
function GetNewPlayerCharacter() -- Line: 891
    -- upvalues: u232 (val), BindableEvent (val), CharacterAnimator (val), u217 (ref), peek (val), Settings (val)
    -- upvalues: u40 (ref)
    if not u232.character or u232.character and u232.character.Parent == nil then
        ...
        if not game.Players.LocalPlayer.Character or not game.Players.LocalPlayer.Character.Parent then
            ...
            if u232.Animator then
                v1 = u232
                local Animator = v1.Animator
                local AltRaycastParams = u217:GetAltRaycastParams()
                Animator:SetRaycastParams(AltRaycastParams)
                v1 = u232
                local Animator_2 = v1.Animator
                v2 = peek
                local v3 = Settings
                v2 = v2(v3.Graphics.ProceduralAnimations)
    ...
    elseif u232.hrp.Parent ~= u232.character and u232.character then
        u232.hrp = u232.character:FindFirstChild("HumanoidRootPart")
        u232.head = u232.character:FindFirstChild("Head")
    end
    if not u40 then
    ...
    elseif u232.hrp and u40.Parent ~= u232.hrp and u232.character and u232.hrp then
        u232.hrp.CanCollide = true
        u40 = u232.hrp:FindFirstChild("RootJoint")
    end
...
function VaultingHandler() -- Line: 946 -- upvalues: u232 (val), CurrentCamera (val), u217 (ref), Resources (val)
    if u232.humanoid.Humanoid.Jump == true and not u232.Vaulted then
        local v1 = u232
        local MoveDirection = v1.humanoid.Humanoid.MoveDirection
        local v2 = CurrentCamera
        local LookVector = v2.CFrame.LookVector
        if 0.1 < (MoveDirection:Dot(LookVector)) and u232.hrp and u232.hrp.Parent then
            local v3
            local v4 = nil
            v2 = nil
            local v5 = nil
            local CFrame_2 = u232.hrp.CFrame
            local v6 = CFrame_2.Position - Vector3.new(0, 2.5, 0)
            local v7 = u217
            local CustomRayDirection = v7.CustomRayDirection
            local v8 = CFrame_2.LookVector * 3
            v7 = CustomRayDirection(v6, v8, true)
            if v7.Instance then
                v8 = v7.Normal:Dot((Vector3.new(0, 1, 0)))
                v3 = math.acos(v8)
                if 1.37 < v3 and v3 < 1.77 then
                    v4 = true
                end
            end
            v3 = CFrame_2.Position - Vector3.new(0, 0, 0)
            v8 = u217
            if v8.CustomRayDirection(v3, CFrame_2.LookVector * 3, true).Instance then
                v2 = true
            end
            local v9 = CFrame_2.Position + Vector3.new(0, 3, 0)
            local v10 = u217
            local CustomRayDirection_3 = v10.CustomRayDirection
            local v11 = CFrame_2.LookVector * 4
            if CustomRayDirection_3(v9, v11, true).Instance then
                v5 = true
            end
            if not u232.States.Crouching and not u232.States.Proning and v4 then
                if v2 and v5 then
                    return
                end
                if u232.hrp:FindFirstChild("RootAttachment") then
                    ...
                    v11 = u232
                    local Humanoid_2 = v11.humanoid.Humanoid
                    local Jumping = Enum.HumanoidStateType.Jumping
                    Humanoid_2:ChangeState(Jumping)
                    u232.humanoid.Animations.Vault:Play()
                    v11 = CFrame_2.LookVector + Vector3.new(0, 200, 0)
                    u232.hrp:ApplyImpulse(v11)
                    u232.Vaulted = true
                    return
                end
            end
...
function SlidingHandler(p1) -- Line: 1001
    -- upvalues: GameState (val), u232 (val), u31 (ref), u145 (ref), u29 (ref), u28 (ref), Resources (val)
    -- upvalues: SharedSprings (val), u36 (ref), u147 (ref)
    local AssemblyLinearVelocity, v1, v2, v3
    ...
    if not u232.States.Sliding
        and not WaterSensor
        and not u232.States.Proning
        and u232.States.Crouching
        and u232.States.Sprinting
        and 10 < AssemblyLinearVelocity.Magnitude then
        v1 = u31
        if u145 <= v1 then
            DrainStamina(u145)
            ...
            v1 = AssemblyLinearVelocity - Vector3.new(0, Y, 0)
            u232.PhysBall:setActive(true, 2)
            local v4 = u232
            local chasis = v4.PhysBall.chasis
            v3 = v1 * 1.5 + Vector3.new(0, -5, 0)
    ...
    if u232.RequestSlideJump and not u29 then
        ...
        if not u232.States.Sliding then
            ...
            v2 = u31
            if not (u147 <= v2) or not isGrounded then
                u232.CrouchPressed = false
                v1 = true
            else
                DrainStamina(u147)
    ...
    if u232.States.Sliding then
        local v5
        if u232.States.Crouching and isGrounded then
            v2 = os.clock()
            if u28 < v2 then
                u29 = false
        ...
        local Magnitude = u232.PhysBall.chasis.AssemblyLinearVelocity.Magnitude
        if WaterSensor then
            ...
            u29 = false
        elseif not u232.States.Crouching then
            v3 = os.clock()
            if not (u28 < v3) then
                if not (Magnitude < 7) then
                    if u232.States.Proning or u232.States.Diving then
                        u232.States.Sliding = false
                        u232.RequestCancel = false
                        if u29 and not u232.States.Diving then
                            Resources.prone_dive_landing:Play()
                    ...
                    elseif not v1 then
                        ...
                        elseif u232.HP <= 0 or not u232.hrp then
                            u232.States.Sliding = false
                            u232.RequestCancel = false
                            if u29 and not u232.States.Diving then
                                Resources.prone_dive_landing:Play()
                ...
                elseif isGrounded then
                    v3 = os.clock()
                    if u28 < v3 or u232.States.Proning or u232.States.Diving then
                        u232.States.Sliding = false
                        u232.RequestCancel = false
                        if u29 and not u232.States.Diving then
                            Resources.prone_dive_landing:Play()
                ...
                elseif u232.States.Proning or u232.States.Diving then
                    u232.States.Sliding = false
                    u232.RequestCancel = false
                    if u29 and not u232.States.Diving then
                        Resources.prone_dive_landing:Play()
        ...
        if not u232.States.Sliding and u232.PhysBall.isActive then
            u232.PhysBall:setActive(false)
            u232.humanoid.Animations.Slide:Stop()
        ...
        if not isGrounded then
            ...
            v5 = math.clamp(v3, 0, 1)
            if not v5 then
                v5 = 0
            end
        end
        slide_loop.Volume = v5
        Resources.slide_loop.PlaybackSpeed = math.clamp(v3, 0.5, 1)
    end
end

function RefreshWalkspeedChanges() -- Line: 1118
    -- upvalues: u37 (ref), u38 (ref), u39 (ref), u232 (val), LocalPlayer (val)
    local v1
    u37 = 0
    u38 = 1
    local v2 = u39
...
v6.CollisionGroup = "NPCCollision"
v6.RespectCanCollide = false
...
function HandleHumanoid(p1) -- Line: 1161
    -- upvalues: u232 (val), LocalPlayer (val), u37 (ref), u38 (ref), GameState (val), u29 (ref)
    ...
    if Humanoid then
        ...
        elseif u232.States.Aiming then
            ...
            if not CurrentWeapon or not CurrentWeapon.Config then
                v1 = 9
            else
        ...
        if typeof(Attribute) ~= "number" or Attribute <= 0 then
            Attribute = 1
        end
        local v3 = v1 + u37
        local v4 = v3 * u38 * Attribute
        local v5 = GameState
        local v6 = v4 * v5.Data.Variables.PlayerSpeed
        v1 = math.max(1, v6)
        ...
        if u232.States.Crouching or u232.States.Sliding then
            v7 = -1.5
        end
        if u232.States.IsDowned then
            v7 = -1.8
        end
        Humanoid.HipHeight = Lerp(Humanoid.HipHeight, v7, p1 * 0.5)
        if Humanoid
            and u232.humanoid.HasLanded == true
            and Humanoid.SeatPart == nil
            and u232.hrp
            and Humanoid.MoveDirection ~= Vector3.new()
            and 0.1 < u232.PlayerMovementUtil.MoveVector.Magnitude
            and not u232.States.Sliding
            and not u29
            and not u232.PlayerMovementUtil.Diving then
            local v8 = u232
            v3 = u232.PlayerVelocity * 0.92 + (u232.hrp.AssemblyLinearVelocity * Vector3.new(1, 0, 1)).magnitude * 0.075
            v5 = math.max(v3, -20)
            v8.PlayerVelocity = math.min(20, v5)
            return
        end
...
function StatesHandler() -- Line: 1247
    -- upvalues: u232 (val), CurrentCamera (val), u359 (ref), u217 (ref), u31 (ref), u146 (ref), Resources (val)
    -- upvalues: u54 (ref)
    local Aiming, Blocking, DoCharging, v1, v2, v3, v4, v5, v6
    ...
    local SprintPressed = u232.SprintPressed
    local Humanoid = u232.humanoid.Humanoid
    local MoveDirection = Humanoid.MoveDirection
    local v7 = CurrentCamera
    local LookVector = v7.CFrame.LookVector
    local v8 = MoveDirection:Dot(LookVector)
    if not u232.hrp then
        v3 = 0
    else
        ...
        v4 = u232
        local lookVector = v4.hrp.CFrame.lookVector
        v3 = MoveDirection_2:Dot(lookVector)
    end
    local IsDowned = u232.States.IsDowned
    if not IsDowned then
        IsDowned = u232.States.IsDead
    end
    if not SprintPressed
        or not (0.1 < u232.PlayerMovementUtil.Direction.Target.Magnitude)
        or not u232.hrp
        or not u232.hrp.Parent
        or not (5 < u232.hrp.AssemblyLinearVelocity.Magnitude)
        or u232.States.Sliding
        or Aiming
        or u232.PlayerMovementUtil.Diving
        or u232.States.Proning
        or u232.States.Crouching
        or IsDowned then
        v4 = false
    else
        v4 = not not u232.MovementEnabled
    end
    if u232.PronePressed and not u232.States.Proning and not u232.States.Diving and u232.hrp and not IsDowned then
        ...
        v5 = true
        if u232.hrp then
            local MoveDirection_3 = u232.humanoid.Humanoid.MoveDirection
            if 0.1 < MoveDirection_3.Magnitude then
                v6 = u217
                local CustomRayDirection = v6.CustomRayDirection
                v1 = u232
                if CustomRayDirection(v1.hrp.Position, MoveDirection_3.Unit * 3, true).Instance then
                    v5 = false
                end
            end
        end
        ...
        elseif v4 or u232.States.Sliding then
            local v9 = u31
            if u146 <= v9 then
                DrainStamina(u146)
                ...
                u232.humanoid:SetJumpPower(30)
                v9 = u232
                local Humanoid_2 = v9.humanoid.Humanoid
                local Jumping = Enum.HumanoidStateType.Jumping
                Humanoid_2:ChangeState(Jumping)
                u232.States.Diving = true
                local AssemblyLinearVelocity = u232.hrp.AssemblyLinearVelocity
                local Y = AssemblyLinearVelocity.Y
                v9 = AssemblyLinearVelocity - Vector3.new(0, Y, 0)
                if u232.States.Sliding and u232.humanoid.HasLanded then
                    ...
                    local Y_2 = AssemblyLinearVelocity_2.Y
                    v9 = AssemblyLinearVelocity_2 - Vector3.new(0, Y_2, 0)
                end
                u232.humanoid.HasLanded = false
                v6 = u232
                local hrp_3 = v6.hrp
                local v10 = v9 * 25
                hrp_3:ApplyImpulse(v10)
            elseif v5 then
                Resources.prone_in:Play()
            end
        elseif v5 then
    ...
    if u232.States.Diving then
        if u232.humanoid.HasLanded or not u232.hrp or u232.humanoid.Climbing or IsDowned then
            if u232.hrp then
                Resources.prone_dive_landing:Play()
            else
                u232.humanoid.HasLanded = true
    ...
    if v4 and u232.States.Proning then
        v4 = false
    end
    ...
    elseif not u232.States.Diving then
        local Prone = u232.humanoid.Animations.Prone
        local PlayerVelocity = u232.PlayerVelocity
        if not (0.5 < v3) then
            v2 = -1
        else
            v2 = 1
        end
        v1 = PlayerVelocity * v2 / Humanoid.WalkSpeed
        Prone:AdjustSpeed(v1)
        if not Prone.IsPlaying then
            Prone:Play(0.2, 1, 1)
        end
        if not u232.hrp or not (3 < u232.hrp.AssemblyLinearVelocity.Magnitude) then
            if Resources.prone_move.Playing and u232.hrp.AssemblyLinearVelocity.Magnitude < 3 then
                Prone:AdjustSpeed(1e-07)
                Resources.prone_move:Stop()
            end
        elseif not Resources.prone_move.Playing then
            Resources.prone_move:Play()
        elseif Resources.prone_move.Playing and u232.hrp.AssemblyLinearVelocity.Magnitude < 3 then
            Prone:AdjustSpeed(1e-07)
            Resources.prone_move:Stop()
        end
    elseif Resources.prone_move.Playing then
        Resources.prone_move:Stop()
    elseif u232.humanoid.Animations.Prone.IsPlaying then
        u232.humanoid.Animations.Prone:Stop(0.0001)
    end
    ...
    if not u232.States.Proning or u232.States.Diving then
        ...
        elseif u232.States.Proning or u232.States.Proning and IsDowned then
            if u232.States.Proning then
                if StanceLocked() then
                    u232.PronePressed = true
                    u232.CrouchPressed = false
    ...
    elseif u232.States.Crouching and not u232.CrouchPressed then
        Resources.crouch_out:Play()
    end
    if not u232.States.Crouching or u232.States.Diving then
        if u232.humanoid.Animations.Crouch.IsPlaying then
            u232.humanoid.Animations.Crouch:Stop()
        end
    elseif not IsDowned then
        local Crouch = u232.humanoid.Animations.Crouch
        local PlayerVelocity_2 = u232.PlayerVelocity
        if not (0.5 < v3) then
            v2 = -1
        else
            v2 = 1
        end
        v1 = PlayerVelocity_2 * v2 / Humanoid.WalkSpeed
        Crouch:AdjustSpeed(v1)
        if not Crouch.IsPlaying then
            Crouch:Play()
        end
    elseif u232.humanoid.Animations.Crouch.IsPlaying then
        u232.humanoid.Animations.Crouch:Stop()
    end
    if u232.States.IsDowned then
        local Downed = u232.humanoid.Animations.Downed
        local v11 = -u232.PlayerVelocity
        if not (0.5 < v3) then
            v2 = -1
        else
            v2 = 1
        end
        v1 = v11 * v2 / Humanoid.WalkSpeed
        Downed:AdjustSpeed(v1)
    ...
    u232.States.Sprinting = v4
    v5 = not u232.ThirdPerson
    if v5 then
        v5 = false
        if v8 < -0.1 then
            v5 = not u232.FocusEnabled
        end
    end
    local States = u232.States
    v6 = v4
    if v6 then
        v6 = true
        if not (u31 <= 0) then
    ...
    v6 = not not u232.CurrentWeapon
    States_2.EquippedGun = v6
    local States_3 = u232.States
    if not u232.CurrentWeapon then
        Blocking = false
    else
    ...
    States_3.Blocking = Blocking
    local States_4 = u232.States
    if not u232.CurrentWeapon then
        DoCharging = false
    else
    ...
    States_4.Charging = DoCharging
end

function updateThirdPerson(p1) -- Line: 1495 -- upvalues: u232 (val)
    local v1 = u232
    local ThirdPerson = v1.ThirdPerson
    u232.ThirdPerson = p1
    if p1 ~= ThirdPerson then
...
function StanceLocked(p1) -- Line: 1503 -- upvalues: u232 (val), u217 (ref)
    ...
    local v2 = u217
    if v2.CustomRayDirection(Position, CFrame.UpVector * v1, true).Instance then
        return true
    end
    local v3 = CFrame.Position + CFrame.LookVector * 1
    local v4 = u217
    if v4.CustomRayDirection(v3, CFrame.UpVector * v1, true).Instance then
        return true
    end
...
local function setupJumpChanged(p1) -- Line: 1526 -- upvalues: u232 (val)
    (p1:GetPropertyChangedSignal("Jump")):Connect(function() -- Line: 1527 -- upvalues: u232 (upval), p1 (val)
...
if Character then
    ...
    ;(Humanoid:GetPropertyChangedSignal("Jump")):Connect(function() -- Line: 1527 -- upvalues: u232 (val), Humanoid (val)
        if not u232.MovementEnabled then
            Humanoid.Jump = false
        end
    end)
...
game.Players.LocalPlayer.CharacterAdded:Connect(function(p1) -- Line: 1538 -- upvalues: u232 (val)
    ...
    ;(Humanoid:GetPropertyChangedSignal("Jump")):Connect(function() -- Line: 1527 -- upvalues: u232 (upval), Humanoid (val)
        if not u232.MovementEnabled then
            Humanoid.Jump = false
        end
    end)
