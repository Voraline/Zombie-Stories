...
return function(p1) -- Line: 66 -- upvalues: u13 (val), peek (val), OnEvent (val), UISounds (val), ButtonFeedback (val)
    local InteractionEnabled, Visible, u478, v1, v2, v3, v4, v5, v6
    ...
    local v7 = if p1.OnClick == nil then if p1.HoverScale == nil then if p1.HoverSheen ~= true then p1.OnHoverChanged ~= nil else true else true else true
    if not v7 then
        v5 = StrokeColor3
    else
        v5 = scope:Computed(function(a1) -- Line: 77 -- upvalues: u21 (val), p1 (val), StrokeColor3 (val)
            if not (a1(u21)) then
                return (a1(StrokeColor3))
            end
            local StrokeHoverColor3 = p1.StrokeHoverColor3
            if not StrokeHoverColor3 then
                StrokeHoverColor3 = StrokeColor3
            end
            return (a1(StrokeHoverColor3))
        end)
    end
    if not p1.HoverScale then
        v6 = nil
    else
        v1 = scope:Computed(function(a1) -- Line: 83 -- upvalues: u21 (val), p1 (val)
            if a1(u21) then
    ...
    local function setHovering(a1) -- Line: 91 -- upvalues: peek (upval), InteractionEnabled (val), u21 (val), p1 (val)
        if not a1 then
            if peek(u21) == a1 then
                return
            end
            ...
            if p1.OnHoverChanged then
                p1.OnHoverChanged(a1)
            end
            return
        end
        if not (peek(InteractionEnabled)) or peek(u21) == a1 then
            return
        end
        u21:set(a1)
        if p1.OnHoverChanged then
    ...
    if p1.InteractionEnabled ~= nil and type(p1.InteractionEnabled) ~= "boolean" then
        v1 = scope:Observer(p1.InteractionEnabled)
        v1:onChange(function() -- Line: 105 -- upvalues: peek (upval), InteractionEnabled (val), u21 (val), p1 (val)
            if peek(InteractionEnabled) or peek(u21) == false then
                return
            end
            u21:set(false)
            if p1.OnHoverChanged then
    ...
    if not p1.HoverSheen then
        ...
        v16.Transparency = NumberSequence.new(v17)
        v13[1] = v14
        v13[2] = v15(v16)
        v12[scope.Children] = v13
        v4 = v4(v12)
    end
    v2[1] = v3
    v2[2] = v9
