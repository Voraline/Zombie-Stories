...
return Red.SharedEvent("SafeKeypad", function(p1) -- Line: 12 -- upvalues: log (val)
    local color, colorName, digit, v1, v2
    ...
    if action ~= "Enable" then
        ...
        if action == "Reset" then
            ...
            return {
                action = action,
                displayText = displayText_3,
                statusText = statusText_3,
                statusColor = statusColor_3,
            }
        end
    ...
    for i, v in ipairs(hints) do
        ...
        if typeof(colorName) == "string" and typeof(digit) == "number" and typeof(color) == "Color3" then
            v1 = {colorName = colorName, digit = digit, color = color}
            table.insert(v3, v1)
            continue
        end
        return nil
    ...
    if codeLength ~= nil then
        if typeof(codeLength) == "number" then
            v1 = math.floor(codeLength)
            codeLength = math.clamp(v1, 1, 32)
        else
            codeLength = nil
        end
    end
