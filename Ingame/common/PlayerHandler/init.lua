...
local ZS_Shared = common:WaitForChild("ZS_Shared")
local Util = ZS_Shared:WaitForChild("Util")
local unseatCharacter = require(Util:WaitForChild("unseatCharacter"))
...
local function relayLocalHealth(p1) -- Line: 78 -- upvalues: u80 (val)
    ...
    local function flush() -- Line: 86 -- upvalues: u10 (ref), u80 (upval)
        ...
        u10 = nil
        if v1 then
            u80.HealthChanged:Fire(v1.HP, v1.Damage, v1.ExtraData)
        end
    end
    ...
    p1.HPChanged:Connect(function(p1, p2) -- Line: 104 -- upvalues: u10 (ref), flush (val)
        ...
        if not v1.FromDamagePacket then
            v1.Damage = math.max((p2 or p1) - p1, 0)
        end
    end)
...
function u80.Init(p1) -- Line: 143 -- upvalues: u80 (val), u76 (val), u58 (val), Players (val), relayLocalHealth (val), u104 (ref), PlayerState (val), u117 (ref), u78 (val), FrameworkEvents (val)
    if u80._Init then
        return
    end
    local function setupPlayerState(p1) -- Line: 148 -- upvalues: u76 (upval), u58 (upval), Players (upval), relayLocalHealth (upval), u80 (upval), u104 (upval)
        local Player = p1.Player
        u76[Player] = p1
        if not u58 and Player == Players.LocalPlayer then
            relayLocalHealth(p1)
        end
        local PropertyChangedSignal = u76[Player]:GetPropertyChangedSignal("IsDead")
        PropertyChangedSignal:Connect(function(p1) -- Line: 154 -- upvalues: Player (val), u80 (upval), u76 (upval), u104 (upval)
            ...
            u80.PlayerDied:Fire(Player, u76[Player])
            if u104 and u104.RecordPlayerDeath then
                u104:RecordPlayerDeath(Player)
            end
        end)
    end
    PlayerState.StateAdded:Connect(setupPlayerState)
    local function PlayerAdded(p1) -- Line: 176 -- upvalues: u58 (upval), setupPlayerState (val), PlayerState (upval), u76 (upval), u80 (upval), u117 (upval), u104 (upval)
        if u58 then
            setupPlayerState(PlayerState.new(p1))
            u76[p1].Died:Connect(function() -- Line: 179 -- upvalues: u80 (upval), p1 (val), u117 (upval)
                u80:Teleport(p1, u117)
            end)
            p1.CharacterAdded:Connect(function(a1) -- Line: 182 -- upvalues: u104 (upval), p1 (val)
                if u104 then
                    u104:UniformPlayer(p1, u104:GetClass(p1))
                end
            end)
        end
    end
    ...
    if u58 then
        ...
        FrameworkEvents.RequestPendingTeleport:SetCallback(function(p1) -- Line: 208 -- upvalues: u78 (upval)
            local v1 = u78[p1]
            u78[p1] = nil
            return v1
        end)
...
function u80.Teleport(p1, p2, p3) -- Line: 219 -- upvalues: u58 (val), u78 (val), ForceTeleport (val), unseatCharacter (val)
    assert(u58, "Teleport only available on server")
    local v1 = typeof(p3)
    if v1 == "CFrame" then
        local v2
        u78[p2] = p3
        if workspace.StreamingEnabled then
            ...
            p2:RequestStreamAroundAsync(Position, 5 + p2:GetNetworkPing() * 2)
        end
        ...
        return
    elseif v1 ~= "Vector3" then
        warn(("[PlayerHandler] Teleport for %s ignored: destination was %s"):format(p2.Name, v1))
        return
    end
end
...
function u80.SetMaxHP(p1, p2, p3, p4) -- Line: 275 -- upvalues: u80 (val)
    local v1 = u80:WaitForPlayerState(p2)
    if not v1 or typeof(p3) ~= "number" or p3 <= 0 then
        return
    end
    v1.MaxHP = p3
    if not p4 then
        v1.HP = math.min(v1.HP, p3)
    else
        v1.HP = p3
    end
    local Character = p2.Character
    ...
    p2:SetAttribute("Skill_CurrentHP", v1.HP)
end
...
function u80.ResetStats(p1, p2) -- Line: 319 -- upvalues: u76 (val), u80 (val), UpdateStatusEvent (val), u75 (val)
    ...
    UpdateStatusEvent:FireClient(p2, u75[p2])
end
...
function u80.SetCameraControllerEnabled(p1, p2, p3) -- Line: 374 -- upvalues: u58 (val), CameraEvent (val)
    if u58 then
        CameraEvent:FireClient(p2, {Type = "SetEnabled", Enabled = p3})
        return
    end
