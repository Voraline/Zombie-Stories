...
if RunService:IsServer() then
    ...
    function u0.SetupDrop(p1, p2, p3) -- Line: 41 -- upvalues: HttpService (val), CollectionService (val), u0 (val), u42 (val), ProgressionEvent (val), u39 (val)
        local u19
        ...
        p1:WaitForChild("Used").OnServerEvent:connect(function(a1) -- Line: 50 -- upvalues: u19 (ref), p1 (val), u42 (upval), p2 (val), ProgressionEvent (upval), u39 (upval)
            if not (u19[a1]) then
                local v1
                local v2 = p1
                ...
                if v1 and 0 < a1.Character.HP.Value then
                    ...
                    p1.Ammo:Play()
                    u42:FillAmmo(a1)
                    game.ReplicatedStorage.common.Remotes.Net:FireClient(a1, "GotAmmo")
                    if p2 ~= a1 then
                        ProgressionEvent:FireClient(p2, {Type = "AddXPItem", Reason = "Resupplied " .. a1.Name, Color = Color3.fromRGB(137, 255, 124)})
                        u39:UpdateXpItem(p2, "Resupply", false, nil, function(p1, p2) -- Line: 64
                            local v1, v2
                            if p1 then
                                v1 = p1
                            else
