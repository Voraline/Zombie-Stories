...
if RunService:IsServer() then
    ...
    function u0.SetupDrop(p1, p2, p3) -- Line: 40
        -- upvalues: HttpService (val), CollectionService (val), u0 (val), ProgressionEvent (val), u39 (val)
        ...
        p1.Used.OnServerEvent:connect(function(p1_2) -- Line: 50 -- upvalues: p3 (ref), p1 (val), p2 (val), ProgressionEvent (upval), u39 (upval)
            if not p3[p1_2] then
                local v1
                local v2 = p1
                ...
                if v1 and 0 < p1_2.Character.HP.Value then
                    p3[p1_2] = true
                    p1.Heal:Play()
                    p1_2.Character.HP.Value = p1_2.Character.MaxHP.Value
                    game.ReplicatedStorage.common.Remotes.Net:FireClient(p1_2, "UsedMedkit")
                    if p2 ~= p1_2 then
                        v1 = ProgressionEvent
                        local v3 = p2
                        local v4 = {
                            Type = "AddXPItem",
                            Reason = "Healed " .. p1_2.Name,
                            Color = Color3.fromRGB(137, 255, 124),
                        }
                        v1:FireClient(v3, v4)
                        v1 = u39
                        v3 = p2
                        v1:UpdateXpItem(v3, "Heal", false, nil, function(p1, p2) -- Line: 64
                            local v1, v2
                            if p1 then
                                v1 = p1
                            else
