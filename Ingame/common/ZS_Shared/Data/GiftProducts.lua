...
local function registerPermanent(p1) -- Line: 23 -- upvalues: MonetizationCatalog (val), u67 (val), u63 (val), u66 (val), u64 (val)
    local v1 = {PassKey = p1.PassKey, PassId = MonetizationCatalog.GetPass(p1.PassKey), EntitlementKey = p1.EntitlementKey}
...
for i, v in ipairs(GiftCatalog.Entries) do
    if v.Kind == "ZBucks" then
        v2 = MonetizationCatalog.GetProduct(v.Key)
        if v2 then
            u63[v2] = true
            u64[v2] = v.DisplayName
        end
    elseif v.Kind ~= "Revive" and v.Kind ~= "Crate" then
        if v.Kind == "Permanent" then
            registerPermanent(v)
...
function u65.IsGiftableProduct(p1) -- Line: 56 -- upvalues: u63 (val)
    local v1 = if typeof(p1) == "number" then u63[p1] == true else false
    return v1
end
