...
local function gradient(p1, p2) -- Line: 10
    return ColorSequence.new({ColorSequenceKeypoint.new(0, p1), ColorSequenceKeypoint.new(1, p2)})
end
...
local v3 = {
    ...
    Chat = {
        Tag = Color3.fromRGB(146, 43, 255),
        Name = Color3.fromRGB(146, 43, 255),
        Body = Color3.fromRGB(102, 247, 255),
    },
    UI = {
...
local v4 = {
    ...
    Chat = {
        Tag = Color3.fromRGB(140, 77, 255),
        Name = Color3.fromRGB(140, 77, 255),
        Body = Color3.fromRGB(193, 145, 255),
    },
    UI = {
...
local v5 = {
    ...
    icon = "rbxassetid://11322093465",
    Chat = {
        Tag = Color3.fromRGB(255, 0, 0),
        Name = Color3.fromRGB(255, 181, 181),
        Body = Color3.fromRGB(255, 181, 181),
    },
    UI = {
...
local v6 = {
    ...
    Chat = {
        Tag = Color3.fromRGB(255, 44, 227),
        Name = Color3.fromRGB(255, 73, 237),
        Body = Color3.fromRGB(255, 143, 229),
    },
    UI = {
...
v2[4] = v6
v2[5] = v7
u0.Definitions = freeze(v2)
local freeze_2 = table.freeze
v2 = {
    Alpha = table.freeze({icon = "rbxassetid://15522534635", Tint = true, Primary = Color3.fromRGB(199, 252, 255)}),
    Verified = table.freeze({icon = "rbxassetid://134830771376709", Tint = false, Primary = Color3.fromRGB(0, 162, 255)}),
    Premium = table.freeze({icon = "rbxassetid://10885647358", Tint = true, Primary = Color3.fromRGB(255, 209, 71)}),
}
u0.Secondary = freeze_2(v2)
local v8 = {}
v2 = {}
...
u0.ById = table.freeze(v8)
u0.ChatIndexById = table.freeze(v2)
...
function u0.ResolveFromRank(p1) -- Line: 90 -- upvalues: u0 (val)
    ...
    for i, v in ipairs(u0.Definitions) do
        if v.minRank and v.minRank <= p1 then
            if v.maxRank and not (p1 <= v.maxRank) then
                continue
            end
            return v.id
        end
...
function u0.ResolveDisplayIcon(p1, p2, p3) -- Line: 104 -- upvalues: u0 (val)
    local v1 = u0.Get(p1)
    if v1.id and v1.icon ~= "" then
        return {Tint = true, Id = v1.id, Image = v1.icon, Color = v1.UI.Primary}
    end
    if p2 and u0.Secondary.Verified.icon ~= "" then
        return {
            Id = "Verified",
            Image = u0.Secondary.Verified.icon,
            Color = u0.Secondary.Verified.Primary,
            Tint = u0.Secondary.Verified.Tint,
        }
    end
    if p3 and u0.Secondary.Premium.icon ~= "" then
        return {
            Id = "Premium",
            Image = u0.Secondary.Premium.icon,
            Color = u0.Secondary.Premium.Primary,
            Tint = u0.Secondary.Premium.Tint,
        }
    end
    return nil
end
