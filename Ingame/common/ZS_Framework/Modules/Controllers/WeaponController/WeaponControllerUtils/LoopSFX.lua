...
function v1.Init(p1, p2) -- Line: 15
    ...
    local Shooting_Start = Config.Shooting_Start
    local Sound = Instance.new("Sound")
    local v1 = Shooting_Start
    local v2 = nil
    local v3 = nil
    for i, j in v1, v2, v3 do
        Sound[i] = j
    ...
    Sound.SoundId = v1 .. Sound.SoundId
    p2.StartSFX = Sound
    local Shooting_Loop = Config.Shooting_Loop
    ...
    p2.LoopSFX = Sound_2
    local Shooting_End = Config.Shooting_End
...
function v1.Start(p1, p2) -- Line: 30 -- upvalues: TweenService (val)
    if not p2.LoopSFX_Playing then
        ...
        local StartSFX = p2.StartSFX
        local v2 = TweenInfo.new(0.01, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
        local v3 = {Volume = p2.StartSFX.Volume}
        v1:Create(StartSFX, v2, v3):Play()
        v1 = TweenService
        local LoopSFX_2 = p2.LoopSFX
        v2 = TweenInfo.new(0.01, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
        v3 = {Volume = p2.LoopSFX.Volume}
        v1:Create(LoopSFX_2, v2, v3):Play()
        p2.LoopSFX:Play()
        p2.StartSFX:Play()
    end
end
...
function v1.Stop(p1, p2) -- Line: 44 -- upvalues: TweenService (val)
    if p2.LoopSFX_Playing then
        ...
        local v1 = TweenService
        local StartSFX = p2.StartSFX
        local v2 = TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
        v1:Create(StartSFX, v2, {Volume = 0}):Play()
        v1 = TweenService
        local LoopSFX = p2.LoopSFX
        v2 = TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
        v1:Create(LoopSFX, v2, {Volume = 0}):Play()
