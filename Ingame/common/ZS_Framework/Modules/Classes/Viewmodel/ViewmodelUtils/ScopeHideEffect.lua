...
local function initialize(p1) -- Line: 13 -- upvalues: u0 (val), u1 (val)
    local Transparency, Transparency_2, Transparency_3, Weld, v1
    local v2 = {}
    u0[p1] = v2
    local v3 = {}
    u1[p1] = v3
    ...
    for i, j in p1:QueryDescendants("BasePart[$HideScope]") do
        table.insert(v2, j)
        Transparency_2 = j.Transparency
        j:SetAttribute("OriginalTransparency", Transparency_2)
        if j.Material ~= Enum.Material.Glass and not j:GetAttribute("NoGlass") then
            v1 = j:Clone()
            v1.Material = "Glass"
            v1.Transparency = v1.Transparency * 0.5
            Transparency_3 = v1.Transparency
            v1:SetAttribute("OriginalTransparency", Transparency_3)
            table.insert(v3, v1)
    ...
    for k, n in p1:QueryDescendants("Texture") do
        if n.Parent:GetAttribute("HideScope") then
            ...
            n:SetAttribute("OriginalTransparency", Transparency)
            table.insert(v2, n)
        end
    end
end
...
local function setPartVisible(p1, p2) -- Line: 52 -- upvalues: u2 (val)
    if not p2 and p1.Parent ~= nil then
        u2[p1] = p1.Parent
        p1.Parent = nil
        return
    end
...
local function correctGlassVisibility(p1) -- Line: 61 -- upvalues: u2 (val)
    ...
    if not v1 and p1.Parent ~= nil then
        u2[p1] = p1.Parent
        p1.Parent = nil
        return
    end
    ...
    for i, j in v8, v9, v10 do
        ...
        if j:IsA("BasePart") and j.Material == Enum.Material.Glass then
            ...
            elseif j.Parent ~= nil then
                u2[j] = j.Parent
                j.Parent = nil
            elseif v2 and j.Parent == nil then
                j.Parent = u2[j]
            end
        end
    end
    ...
    for k, n in v9, v10, v11 do
        ...
        if v8 ~= 0 then
            ...
            elseif n.Parent ~= nil then
                u2[n] = n.Parent
                n.Parent = nil
            elseif v3 and n.Parent == nil then
                n.Parent = u2[n]
            end
        elseif n.Parent ~= nil then
            u2[n] = n.Parent
