...
function v1.set(p1, p2) -- Line: 67 -- upvalues: isSimilar (val), change (val)
    ...
    if not isSimilar(_EXTREMELY_DANGEROUS_usedAsValue, p2) then
...
return function(p1, p2) -- Line: 32 -- upvalues: External (val), u34 (val), nicknames (val)
    local v1 = os.clock()
    if p2 == nil then
        if typeof(p1) ~= "table" or p1[1] == nil and next(p1) ~= nil then
    ...
    local v3 = u34
    local u30 = setmetatable(v2, v3)

    function v2() -- Line: 51 -- upvalues: u30 (val)
        u30.scope = nil
    end
